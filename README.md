![Status do Projeto](https://img.shields.io/badge/Status-Em_Desenvolvimento-yellow) <br>
![Tecnologia](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![Tecnologia](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)

````markdown
# 🏠 BlueStay

> Plataforma de aluguel de apartamentos por temporada com experiência estilo Airbnb.


## 💻 Sobre o Projeto

Este projeto é o trabalho final da disciplina de Desenvolvimento de Interfaces para Web.
O objetivo é desenvolver o front-end completo de uma aplicação de reservas, simulando um cenário real de mercado.

### ⚠️ O Grande Desafio Técnico
Para fins acadêmicos e de aprendizado profundo das bases do CSS, este projeto foi construído com uma restrição severa:

* 🚫 Proibido o uso de `display: flex`
* 🚫 Proibido o uso de `display: grid`

Todo o layout responsivo e posicionamento foi construído utilizando técnicas clássicas como Float, Position e Box Model.

## 🎨 Identidade Visual

* Nome: BlueStay
* Cores Principais: Royal Blue (`#2196F3`) e Chambray (`#3F5388`).
* Conceito: Minimalismo, confiança e clareza (Inspirado no Airbnb).

## ☑️ Funcionalidades & Páginas (Escopo)

O projeto conta com 8 páginas e seções complexas:

- [ ] Home: Página inicial com busca e destaques.
- [ ] Listagem: Busca de apartamentos (Layout em colunas com Float).
- [ ] Detalhes do Apartamento: Galeria de fotos e informações.
- [ ] Quem Somos: História da empresa.
- [ ] Login: Acesso do usuário.
- [ ] Suporte / FAQ: Dúvidas frequentes e segurança.
- [ ] Guia Turístico: Dicas de Maceió, RJ e POA.
- [ ] Contato: Formulário e localização.

## 🚀 Como rodar o projeto

1. Clone este repositório:

git clone https://github.com/Wolsadek/BlueStay-frontend.git

````

2.  Entre na pasta:

<!-- end list -->

```bash
cd BlueStay-frontend
```

3.  Abra o arquivo `index.html` no seu navegador preferido.

## 🤝 Equipe

  * **Lorenzo** - [GitHub](https://github.com/Wolsadek)
  * **Filipe** - [GitHub](https://github.com/1xPimentelx1)
  * **Miguel** - [GitHub](https://github.com/Mdornelles)
  * **Guilherme** - [GitHub](https://github.com/Calixtoz1k)

-----

Desenvolvido com 💙 para a disciplina de Desenvolvimento de Interfaces para Web.
